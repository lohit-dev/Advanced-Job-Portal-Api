...
impl UserRepository for UserService {
    async fn get_user(
        ...
        if let Some(user_id) = user_id {
            let row = sqlx::query(
                "SELECT id, name, email, password, verified, created_at, updated_at, verification_token, token_expires_at, role FROM users WHERE id = $1"
            )
            .bind(user_id)
            ...
            let row = sqlx::query(
                "SELECT id, name, email, password, verified, created_at, updated_at, verification_token, token_expires_at, role FROM users WHERE name = $1"
            )
            .bind(name)
            .fetch_optional(&self.db)
            .await?;
            ...
            let row = sqlx::query(
                "SELECT id, name, email, password, verified, created_at, updated_at, verification_token, token_expires_at, role FROM users WHERE email = $1"
            )
            .bind(email)
            .fetch_optional(&self.db)
            .await?;
            ...
            let row = sqlx::query(
                "SELECT id, name, email, password, verified, created_at, updated_at, verification_token, token_expires_at, role FROM users WHERE verification_token = $1"
            )
            .bind(token)
            .fetch_optional(&self.db)
            .await?;
    ...
    async fn get_users(&self, page: u32, limit: usize) -> Result<Vec<User>, sqlx::Error> {
        ...
        let rows = sqlx::query(
            "SELECT id, name, email, password, verified, created_at, updated_at, verification_token, token_expires_at, role FROM users ORDER BY created_at DESC LIMIT $1 OFFSET $2"
        )
        .bind(limit as i64)
        .bind(offset as i64)
        .fetch_all(&self.db)
